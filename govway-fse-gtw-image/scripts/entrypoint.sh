...
declare -r JDBC_AWS_INIT_FILE="${CATALINA_HOME}/conf/fix_jdbcaws_init"
...
"
    ...
    postgresql)
        ...
        if [ "${FSE_CLOUD_PROVIDER}" == "aws" ]
        then
            if [ -n "${GOVWAY_DS_CONN_PARAM}" ]
            then
                GOVWAY_DS_CONN_PARAM="${GOVWAY_DS_CONN_PARAM}&wrapperPlugins=iam"
            else
                GOVWAY_DS_CONN_PARAM='wrapperPlugins=iam'
            fi
...
if [ ! -f "${JDBC_AWS_INIT_FILE}" -a "${FSE_CLOUD_PROVIDER}" == "aws" ]
then
          
    JDBC_RUN_URL='jdbc:aws-wrapper:postgresql://${GOVWAY_DB_SERVER}/${GOVWAY_DB_NAME}${DATASOURCE_CONN_PARAM}'
    JDBC_CONF_URL='jdbc:aws-wrapper:postgresql://${GOVWAY_CONF_DB_SERVER}/${GOVWAY_CONF_DB_NAME}${DATASOURCE_CONF_CONN_PARAM}'
    JDBC_TRAC_URL='jdbc:aws-wrapper:postgresql://${GOVWAY_TRAC_DB_SERVER}/${GOVWAY_TRAC_DB_NAME}${DATASOURCE_TRAC_CONN_PARAM}'
    JDBC_STAT_URL='jdbc:aws-wrapper:postgresql://${GOVWAY_STAT_DB_SERVER}/${GOVWAY_STAT_DB_NAME}${DATASOURCE_STAT_CONN_PARAM}'
